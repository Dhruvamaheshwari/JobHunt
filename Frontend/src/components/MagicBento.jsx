...
const MagicBento = ({
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8 bento-section" ref={gridRef}>
        {jobs.map((job) => {
          ...
          const cardProps = {
            className: `magic-bento-card flex flex-col justify-between h-full group transition-all duration-200 cursor-pointer ${
          ...
          return (
            ...
            >
              {/* Job Card Inner Layout */}
              <div className="flex flex-col justify-between h-full space-y-4">
                <div className="space-y-3">
                  {/* Title & Source Badge */}
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="font-bold text-slate-900 dark:text-slate-100 leading-snug text-base group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                      {job.title}
                    </h3>
                    <Badge variant="default" className="shrink-0 font-semibold text-[11px] px-2 py-0.5">
                  ...
                  {/* Location, Job Type & Date */}
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs text-slate-600 dark:text-slate-400 font-medium">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <MapPin className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 shrink-0" />
                      <span className="break-words">{job.location || 'Remote'}</span>
                    </div>
                    {job.jobType && (
                      <div className="flex items-center gap-1 text-slate-500 dark:text-slate-400 capitalize">
                  ...
                  {/* Candidate Job Description Snippet */}
                  {cleanDescription && (
                    <div className="border-t border-slate-100 dark:border-slate-800/80 pt-2.5 mt-2">
                ...
                {/* Footer Badges & Apply Link */}
                <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800/80 pt-3 mt-auto z-10 gap-2">
                  <Badge variant="secondary" className="capitalize text-[11px] font-medium px-2 py-0.5">
                    {job.category || 'General'}
                  </Badge>

                  {job.url ? (
                    <a href={job.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}>
                      <Button size="sm" className="gap-1.5 text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white shrink-0">
